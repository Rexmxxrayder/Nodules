...
public class WindShield : EntityBasic {
    protected override void LoadSetup() {
        base.LoadSetup();
        GetRootComponent<EntityMainCollider2D>().OnCollisionEnterDelegate += ReflectProjectile;
    }
    ...
    public void ReflectProjectile(Collision2D collision) {
        if (collision.gameObject.GetRoot().CompareTag("Projectile")){
            ...
            Vector3 velocity = collision.gameObject.GetRootComponent<EntityPhysics>().Velocity;
